...
from rotaterect import four2eight, eight2four
...
class LoadImages:  # for inference
    ...
    def __next__(self):
        ...
        img = np.ascontiguousarray(img)     #将内存存储不连续变为连续，加快处理
    ...
    def __getitem__(self, index):
        ...
        if mosaic:
            ...
            if x.size > 0:
                ...
                labels = x.copy()
                labels = four2eight(labels)
                labels[:, 1] = ratio[0] * w * labels[:, 1] + pad[0]  # pad width
                labels[:, 2] = ratio[1] * h * labels[:, 2] + pad[1]  # pad height
                labels[:, 3] = ratio[0] * w * labels[:, 3] + pad[0]  # pad width
                labels[:, 4] = ratio[1] * h * labels[:, 4] + pad[1]  # pad height
                labels[:, 5] = ratio[0] * w * labels[:, 5] + pad[0]  # pad width
                labels[:, 6] = ratio[1] * h * labels[:, 6] + pad[1]  # pad height
                labels[:, 7] = ratio[0] * w * labels[:, 7] + pad[0]  # pad width
                labels[:, 8] = ratio[1] * h * labels[:, 8] + pad[1]  # pad height
...
def load_mosaic(self, index):
    ...
    for i, index in enumerate(indices):
        ...
        if x.size > 0:  # Normalized xywh to pixel xyxy format
            labels[:, 1] = w * labels[:, 1] + padw
            labels[:, 2] = h * labels[:, 2] + padh
            labels[:, 3] = w * labels[:, 3] + padw
            labels[:, 4] = h * labels[:, 4] + padh
            labels[:, 5] = w * labels[:, 5] + padw
            labels[:, 6] = h * labels[:, 6] + padh
            labels[:, 7] = w * labels[:, 7] + padw
            labels[:, 8] = h * labels[:, 8] + padh
...
def random_perspective(img, targets=(), degrees=10, translate=.1, scale=.1, shear=10, perspective=0.0, border=(0, 0)):
    ...
    if n:
        ...
        x_center = np.mean(
            np.concatenate((np.max(xy[:, [0, 2, 4, 6]], 1).reshape(n, 1), np.min(xy[:, [0, 2, 4, 6]], 1).reshape(n, 1)),
                           1), 1)
        y_center = np.mean(
            np.concatenate((np.max(xy[:, [1, 3, 5, 7]], 1).reshape(n, 1), np.min(xy[:, [1, 3, 5, 7]], 1).reshape(n, 1)),
                           1), 1)
        ...
        targets = targets[i]
        targets[:, 1:9] = xy[i]
        targets = eight2four(targets[:,0:9])

...
from rotaterect import four2eightnc, oriented2horizontal, whloss, whloss2, orient_iou, distloss
...
def compute_loss(p, targets, model):  # predictions, targets, model
    ...
    lagl = torch.zeros(1, device=device)
    tcls, tbox, indices, anchors, thetas= build_targets(p, targets, model)  # targets
    h = model.hyp  # hyperparameters

    # Define criteria
    BCEcls = nn.BCEWithLogitsLoss(pos_weight=torch.Tensor([h['cls_pw']])).to(device)
    BCEobj = nn.BCEWithLogitsLoss(pos_weight=torch.Tensor([h['obj_pw']])).to(device)
    SMlagl = nn.SmoothL1Loss().to(device)
    ...
    for i, pi in enumerate(p):  # layer index, layer predictions
        ...
        if n:
            ...
            # Regression
            ang = ps[:, 4].sigmoid() * 3. - 1.5
            pxy = ps[:, :2].sigmoid() * 2. - 0.5
            pwh = (ps[:, 2:4].sigmoid() * 2) ** 2 * anchors[i]
            pbox = torch.cat((pxy, pwh), 1).to(device)
            # ppbox, tgbox = oriented2horizontal(pbox, ang).to(device), oriented2horizontal(tbox[i], thetas[i]).to(device)
            iou = bbox_iou(pbox.T, tbox[i], x1y1x2y2=False, CIoU=True)  # iou(prediction, target)
            # iou = orient_iou(ppbox, tgbox, device= device)
            # dl = distloss(pbox.T, tbox[i])
            # whl = whloss2(pwh, tbox[i][:, 2:4])
            # iou = - (whl + dl)
            ...
            lagl += SMlagl(ang, thetas[i]).mean()  # angloss
    ...
    loss = lbox + lobj + lcls +lagl
    return loss * bs, torch.cat((lbox, lobj, lcls, lagl, loss)).detach()
...
def build_targets(p, targets, model):
    ...
    for i in range(det.nl):
        ...
        if nt:
            ...
            j = torch.max(r, 1. / r).max(2)[0] < model.hyp['anchor_t']  # compare 找到与target面积接近的anchor
    ...
    return tcls, tbox, indices, anch, thetas
...
def non_max_suppression(prediction, conf_thres=0.1, iou_thres=0.6, merge=False, classes=None, agnostic=False):
    ...
    """
    # prediction = 1*25200*11, 11 = x,y,w,h,@,conf,5cls
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        # Box (center x, center y, width, height, @) to (x1, y1, x2, y2....x4,y4)
        box = xywh2xyxy(x[:, :4])

        # Detections matrix nx6 (xyxy, conf, cls)
        if multi_label:
            i, j = (x[:, 6:] > conf_thres).nonzero(as_tuple=False).T
            # 表示第i个bbox的从第1个class开始数的第j个cls的置信度>阈值
            # x = box+x[i]的cls的置信度+x[i]的第几个cls
            x = torch.cat((box[i], x[i, 4, None], x[i, j + 6, None], (j[:, None]).float()), 1)
...
def plot_one_box(x, img, color=None, label=None, line_thickness=None):
    ...
    Pts = np.array([c4, c1, c2, c3], np.int32)
    Pts = Pts.reshape((-1, 1, 2))
    cv2.polylines(img, [Pts], True, color, thickness=2, lineType=cv2.LINE_AA)
    label1 = False
    if label1:
...
def plot_images(images, targets, paths=None, fname='images.jpg', names=None, max_size=640, max_subplots=16):
    ...
    for i, img in enumerate(images):
        ...
        if len(targets) > 0:
            ...
            boxes = four2eightnc(image_targets[:, 2:6],image_targets[:, 6]).T
...
def plot_results(start=0, stop=0, bucket='', id=(), labels=(), save_dir=''):
    ...
    ax = ax.ravel()
    s = ['Box', 'Objectness', 'Classification', 'angle', 'Precision', 'Recall',
         'val Box', 'val Objectness', 'val class', 'mAP@0.5', 'mAP@0.5:0.95']
    if bucket:
    ...
    for fi, f in enumerate(files):
        try:
            results = np.loadtxt(f, usecols=[2, 3, 4, 5, 9, 10, 12, 13, 10, 11], ndmin=2).T
            n = results.shape[1]  # number of rows
            x = range(start, min(stop, n) if stop else n)
            for i in range(10):
                y = results[i, x]

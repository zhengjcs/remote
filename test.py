...
from rotaterect import four2eightnc, pbox_iou, xywh2, clip_coord
...
def test(data,
    ...
    for batch_i, (img, targets, paths, shapes) in enumerate(tqdm(dataloader, desc=s)):
        ...
        with torch.no_grad():
            # Run model
            t = time_synchronized()
            # inf = 1*25200*11  train =list3*[1*3*80*80*11]
            ...
            if nl:
                ...
                for cls in torch.unique(tcls_tensor):
                    ...
                    if pi.shape[0]:
                        ...
                        # pbox = xyxy2xywh(pred[pi, :4])
                        # ppbox = four2eightnc(pbox, pred[pi, 4])
                        # ttbox = four2eightnc(tbox[ti, :4], tbox[ti, 4])
                        ious, i = box_iou(pred[pi, :4], tbox[ti]).max(1)  # best ious, indices
                        # ious, i = pbox_iou(ppbox, ttbox, device= device).max(1)
        ...
        if plots and batch_i < 1:
            ...
            output = output_to_target(output, width, height)
            plot_images(img, output, paths, str(f), names)  # predictions
...
if __name__ == '__main__':
    ...
    parser.add_argument('--weights', nargs='+', type=str, default='besthrsc.pt', help='model.pt path(s)')
    parser.add_argument('--data', type=str, default='data/VOCh.yaml', help='*.data path')
